 ...
 */
export function formatUnitCost(amount: number, symbol: string = '$'): string {
 ...
 */
export function calculateSellingPriceFromMargin(cost: number, targetMarginPercent: number): number {
  if (cost <= 0) return 0;
 ...
 */
export function calculateProfitMarginPercent(cost: number, price: number): number {
 ...
 * Formula: Price / Cost
 */
export function calculateMarkupMultiplier(cost: number, price: number): number {
  if (cost <= 0) return 0;
